...
while True:
    ...
    if op.multi_hand_landmarks:
        for i in op.multi_hand_landmarks:
            ...
            x, y = int(i.landmark[8].x * screen_width), int(i.landmark[8].y * screen_height)
            ...
            if curr_tool == "draw":
                xi, yi = int(i.landmark[12].x * screen_width), int(i.landmark[12].y * screen_height)
                y9 = int(i.landmark[9].y * screen_height)
    ...
    op = cv2.bitwise_and(frm, frm, mask=mask.astype('uint8'))
    frm[:, :, 1] = op[:, :, 1]
    frm[:, :, 2] = op[:, :, 2]
